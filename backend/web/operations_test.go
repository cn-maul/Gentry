...
func TestDetectionFingerprintCoversFullFieldSemantics(t *testing.T) {
	fields := []fieldRequest{{Name: "title", Selector: ".title", Type: "text", Attr: "", Transform: "trim"}}
	base := computeDetectionFingerprint("https://example.com", "body", "", fields, "")
	changedAttr := append([]fieldRequest(nil), fields...)
	changedAttr[0].Attr = "data-title"
	if base == computeDetectionFingerprint("https://example.com", "body", "", changedAttr, "") {
	...
	if base == computeDetectionFingerprint("https://example.com", "body", "", changedTransform, "") {
...
func TestDetectionFingerprintCanonicalizesOrder(t *testing.T) {
	...
	fpA := computeDetectionFingerprint("https://example.com", "body", "", fieldsA, "")
	fpB := computeDetectionFingerprint("https://example.com", "body", "", fieldsB, "")
	if fpA != fpB {
		t.Error("field order should not change the detection fingerprint")
	}
}

func TestDetectionFingerprintIncludesFetchConfig(t *testing.T) {
	fields := []fieldRequest{{Name: "title", Selector: "name", Type: "text"}}
	base := computeDetectionFingerprint("https://shop.example/product", "data", "*", fields, `{"mode":"api_json","url":"https://shop.example/api?id=1","items_path":"data"}`)
	changed := computeDetectionFingerprint("https://shop.example/product", "data", "*", fields, `{"mode":"api_json","url":"https://shop.example/api?id=2","items_path":"data"}`)
	if base == changed {
